...
/// @author 2013 Alejandro Morell Garica (http://github.com/amorellgarcia)   ///
////////////////////////////////////////////////////////////////////////////////
...
#define SRUTIL_DELEGATE_INVOKER_CLASS_NAME SRUTIL_DELEGATE_JOIN_MACRO(delegate_invoker,SRUTIL_DELEGATE_PARAM_COUNT)
	...
	public:
...
#ifdef SRUTIL_DELEGATE_PREFERRED_SYNTAX
		typedef SRUTIL_DELEGATE_INVOKER_CLASS_NAME<R (SRUTIL_DELEGATE_TEMPLATE_ARGS)> invoker_type;
#else
		...
		template <return_type (*TMethod)(void * SRUTIL_DELEGATE_SEPARATOR SRUTIL_DELEGATE_TEMPLATE_ARGS)>
		static SRUTIL_DELEGATE_CLASS_NAME from_function_data(void *data) {
			return from_stub(data, &function_data_stub<TMethod>);
		}
		...
		size_t printTo(Print &p) const {
			size_t s = 0;
			if (object_ptr != NULL) {
				s += p.print((uintptr_t)object_ptr, 16);
				s += p.print(' ');
			}
			s += p.print((uintptr_t)stub_ptr, 16);
			return s;
		}
	...
	private:
		...
		template <return_type (*TMethod)(void * SRUTIL_DELEGATE_SEPARATOR SRUTIL_DELEGATE_TEMPLATE_ARGS)>
		static return_type SRUTIL_DELEGATE_CALLTYPE function_data_stub(void *data SRUTIL_DELEGATE_SEPARATOR SRUTIL_DELEGATE_PARAMS) {
			return (TMethod)(data SRUTIL_DELEGATE_SEPARATOR SRUTIL_DELEGATE_ARGS);
		}
